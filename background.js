...
async function openSearchTabs(query, engine, multiTab) {
  const urls = [
    SEARCH_ENGINES[engine].url(query)
  ];
...
chrome.webRequest.onBeforeRequest.addListener(
  function(details) {
    return new Promise((resolve) => {
      storage.local.get(['enabled', 'multiTab', 'stats', 'searchEngine', 'whitelist', 'blacklist'], function(data) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'closePerplexityAndFocusGoogle') {
    ...
    chrome.tabs.query({}, (tabs) => {
      const match = tabs.find(t => t.url.includes('https://www.google.com/search') && t.url.includes(query));
      if (match) {
