document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateSearchHistory() {
    storage.local.get('searchHistory', function(data) {
      ...
      history.slice(-5).reverse().forEach(item => {
        ...
        itemDiv.style.cssText = 'margin-bottom: 8px; padding: 8px; background: var(--bg-primary); border-radius: 4px;';
  ...
  saveListBtn.addEventListener('click', () => {
    const wList = whitelistInput.value.split(',').map(w => w.trim().toLowerCase()).filter(Boolean);
    const bList = blacklistInput.value.split(',').map(b => b.trim().toLowerCase()).filter(Boolean);
